...
class TransformerBinaryBalancedTrainingDataset(TransformerTrainingDataset):
    """
    Balanced sampling dataset for BINARY classification (single label).
    Handles the case where there's only one target label (e.g., DE for dementia detection).
    Samples equally from positive (label=1) and negative (label=0) classes.
    """
    ...
    def __init__(self,
        ...
        # Get the single label key
        self.label_key = list(self.tgt_modalities.keys())[0]

        # Separate indices by class
        self.positive_indices = []  # label = 1
        self.negative_indices = []  # label = 0

        for i, smp in enumerate(tgt):
            if smp[self.label_key] == 1:
                self.positive_indices.append(i)
            else:
                self.negative_indices.append(i)

        print(f"[BinaryBalancedDataset] Label: {self.label_key}")
        print(f"[BinaryBalancedDataset] Positive samples: {len(self.positive_indices)}")
        print(f"[BinaryBalancedDataset] Negative samples: {len(self.negative_indices)}")
    ...
    def __getitem__(self,
        ...
        # Randomly select class (50/50 probability)
        if random.random() < 0.5:
            # Sample from positive class
            idx = random.choice(self.positive_indices)
        else:
            # Sample from negative class
            idx = random.choice(self.negative_indices)
