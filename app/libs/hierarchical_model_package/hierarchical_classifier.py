...
# Adjusted path: nmed2024 is now in the same libs folder as the package
NMED_PATH = PACKAGE_DIR.parent / "nmed2024"
if NMED_PATH.exists():
    sys.path.insert(0, str(NMED_PATH))
    sys.path.insert(0, str(NMED_PATH / "dev"))
elif (PACKAGE_DIR / "nmed2024").exists():
    # Alternative location if it's nested
    NMED_PATH = PACKAGE_DIR / "nmed2024"
    sys.path.insert(0, str(NMED_PATH))
    sys.path.insert(0, str(NMED_PATH / "dev"))
...
class HierarchicalCognitiveClassifier:
    ...
    def predict(
        ...
        for idx in range(len(data)):
            ...
            # Include raw features for SHAP/Explainability
            result['raw_features'] = sample.iloc[0].to_dict()
            
            # Include original ID if present in the data
            if 'ID' in sample.columns:
                result['original_id'] = str(sample.iloc[0]['ID'])
