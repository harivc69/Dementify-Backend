...
# from .MRI_load import load_mris  # Lazy import to avoid heavy dependencies
...
value_mapping = {
    # Commented out for integer-based NACC data
...
class CSVDataset:
        ...
        for fea in self.cnf['feature'].keys():
            ...
            if self.cnf['feature'][fea]['type'] == 'imaging':
                ...
                # load MRI embeddings 
                    ...
                    print('img_mode is 2')
                    from .MRI_load import load_mris  # Lazy import only when needed
    ...
    # Aliases for compatibility with ADRDModel
    @property
    def src_modalities(self):
        ''' Alias for feature_modalities (source modalities for transformer) '''
        return self.feature_modalities

    @property
    def tgt_modalities(self):
        ''' Alias for label_modalities (target modalities for transformer) '''
        return self.label_modalities
