...
# DISABLED: wandb import hangs in WSL2
# import wandb
import torch
...
from ..utils import TransformerTrainingDataset, TransformerBalancedTrainingDataset, TransformerValidationDataset, TransformerTestingDataset, Transformer2ndOrderBalancedTrainingDataset, TransformerBinaryBalancedTrainingDataset
...
def _manage_ctx_fit(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print("TESTING")
        print("KWARGS")
        print(kwargs['self'].device)
        print(kwargs['self']._device_ids)
        print(kwargs['self'].cuda_devices)
        #print(**kwargs)
...
class ADRDModel(BaseEstimator):
    ...
    # def fit(self, x, y) -> Self:
        ...
        # DISABLED: wandb causing hangs in WSL2
        print("DEBUG: Skipping wandb initialization (disabled for WSL2 compatibility)...")
        # if self.wandb_ == 1:
        #     wandb.init(
        ...
        #     wandb.run.log_code(".")
        # else:
        #     print("DEBUG: Initializing wandb in disabled mode...")
        #     wandb.init(mode="disabled")
        #     print("DEBUG: wandb initialized")
        # for PyTorch computational efficiency
        print("DEBUG: Setting torch num_threads...")
        torch.set_num_threads(1)
        print("DEBUG: torch num_threads set")
            ...
            for k, info in self.src_modalities.items():
                if info['type'] == 'imaging':
                    ...
                    if isinstance(self.img_size, (tuple, list)):
                        img_shape = (1,) + tuple(self.img_size)
                    else:
                        img_shape = (1, self.img_size, self.img_size, self.img_size)

                    if 'emb' not in self.img_net.lower():
                        info['shape'] = img_shape
                        info['img_shape'] = img_shape
                    elif 'swinunetr' in self.img_net.lower():
                        info['shape'] = (1, 768, 4, 4, 4)
                        info['img_shape'] = (1, 768, 4, 4, 4)
        print("DEBUG: img_mode check completed")
        


        print("DEBUG: Calling _init_net()...")
        self._init_net()
        print("DEBUG: _init_net() completed")

        print("DEBUG: Calling _init_dataloader()...")
        ...
        print("DEBUG: _init_dataloader() completed")
        ...
        print("DEBUG: Initializing focal losses...")
        self.loss_fn = {}
        ...
        print("DEBUG: Focal losses initialized")
        ...
        print("DEBUG: Initializing ranking loss...")
        self.lambda_coeff = 0.005
        self.margin = 0.25
        self.margin_loss = torch.nn.MarginRankingLoss(reduction='sum', margin=self.margin)
        print("DEBUG: Ranking loss initialized", flush=True)

        # Initialize metrics history for logging
        metrics_history = []
        metrics_file = self.ckpt_path.replace('.pt', '_metrics.json') if self.ckpt_path else None

        # training loop
        print(f"DEBUG: Starting training loop for {self.num_epochs} epochs...", flush=True)
        for epoch in range(self.start_epoch, self.num_epochs):
            print(f"DEBUG: Starting epoch {epoch+1}/{self.num_epochs}", flush=True)
            met_trn = self.train_one_epoch(ldr_trn, epoch)
            print(f"DEBUG: Training epoch {epoch+1} completed", flush=True)
            met_vld = self.validate_one_epoch(ldr_vld, epoch)
            print(f"DEBUG: Validation epoch {epoch+1} completed", flush=True)

            # Print validation metrics for monitoring
            avg_auroc = np.mean([met_vld[i]["AUC (ROC)"] for i in range(len(self.tgt_modalities))])
            avg_aupr = np.mean([met_vld[i]["AUC (PR)"] for i in range(len(self.tgt_modalities))])
            avg_loss = np.mean([met_vld[i]["Loss"] for i in range(len(self.tgt_modalities))])
            avg_trn_loss = np.mean([met_trn[i]["Loss"] for i in range(len(self.tgt_modalities))])
            print(f"  Epoch {epoch+1}: Val Loss={avg_loss:.4f}, AUC-ROC={avg_auroc:.4f}, AUC-PR={avg_aupr:.4f}", flush=True)
            ...
            epoch_metrics = {
                ...
                'val_auroc': float(avg_auroc),
                'val_aupr': float(avg_aupr),
                'per_class_metrics': {
                    list(self.tgt_modalities.keys())[i]: {
                        'auroc': float(met_vld[i]["AUC (ROC)"]),
                        'aupr': float(met_vld[i]["AUC (PR)"]),
                        'loss': float(met_vld[i]["Loss"])
            ...
            if metrics_file:
                ...
                with open(metrics_file, 'w') as f:
                    json.dump({'epochs': metrics_history, 'best_auroc': best_crit, 'best_aupr': best_crit_AUPR}, f, indent=2)

            print(self.ckpt_path.split('/')[-1], flush=True)
            ...
            if is_better_AUPR:
                ...
                if self.save_intermediate_ckpts:
                    print(f"  [NEW BEST AUPR] Saving model to {self.ckpt_path[:-3]}_AUPR.pt...", flush=True)
                    self.save(self.ckpt_path[:-3]+"_AUPR.pt", epoch)
            if is_better:
                best_crit = curr_crit
                best_state_dict = deepcopy(self.net_.state_dict())
                if self.save_intermediate_ckpts:
                    print(f"  [NEW BEST AUROC] Saving model to {self.ckpt_path}...", flush=True)
    ...
    def train_one_epoch(self, ldr_trn, epoch):
        ...
        for n_iter, (x_batch, y_batch, mask, y_mask) in enumerate(ldr_trn):
            ...
            with torch.autocast(
                ...
                for i, k in enumerate(self.tgt_modalities):
                    ...
                    # Guard against zero mask sum (rare classes may have no samples in batch)
                    mask_sum = y_mask[k].sum()
                    if mask_sum > 0:
                        msk_loss_mean = msk_loss_task.sum() / mask_sum
                    else:
                        msk_loss_mean = 0.0  # Skip this label if no valid samples
        ...
        # add loss to metrics (guard against empty loss lists for rare classes)
        for i in range(len(self.tgt_modalities)):
            if len(losses_trn[i]) > 0:
                met_trn[i]['Loss'] = np.mean(losses_trn[i])
            else:
                met_trn[i]['Loss'] = 0.0  # No valid samples for this class
        
        # log metrics to wandb (DISABLED for WSL2)
    ...
    def validate_one_epoch(self, ldr_vld, epoch):
        ...
        # add loss to metrics (guard against empty loss lists for rare classes)
        for i in range(len(self.tgt_modalities)):
            if len(losses_vld[i]) > 0:
                met_vld[i]['Loss'] = np.mean(losses_vld[i])
            else:
                met_vld[i]['Loss'] = 0.0  # No valid samples for this class
...
#            state_dict = self.net_.module.state_dict()
    ...
    def load(self, filepath: str, map_location: str = 'cpu', img_dict=None) -> None:
        ...
        print("Len of state_dict src modalities")
        print(len(state_dict['src_modalities']))
        #for key in list(state_dict['src_modalities'].keys()):
        #    if key not in self.src_modalities.keys():
        #        del state_dict['src_modalities'][key]
        #print(len(state_dict['src_modalities']))
    ...
    def _init_dataloader(self, x_trn, x_vld, y_trn, y_vld, img_train_trans=None, img_vld_trans=None):
        ...
        if self.balanced_sampling:
            # Check if binary classification (single label) or multi-class
            num_labels = len(self.tgt_modalities)
            if num_labels == 1:
                # Binary classification - use binary balanced dataset
                print(f"[ADRDModel] Using TransformerBinaryBalancedTrainingDataset for binary classification")
                dat_trn = TransformerBinaryBalancedTrainingDataset(
                    x_trn, y_trn,
                    self.src_modalities,
                    self.tgt_modalities,
                    dropout_rate = .5,
                ...
                # Multi-class classification - use 2nd order balanced dataset
                print(f"[ADRDModel] Using Transformer2ndOrderBalancedTrainingDataset for {num_labels}-class classification")
